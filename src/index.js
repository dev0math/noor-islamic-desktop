...
app.setAppUserModelId("org.noor.Noor");
const appInitialization = require('./modules/appInitialization.js');
const App_Path = path.join(app?.getPath("appData"), './noor');
...
app.on('ready', async (e) => {
    ...
    win?.once('ready-to-show', () => {
        ...
        if (process.argv.includes('--hidden')) {
            console.log("[Noor-CLI] Hidden & Minimized.")
            win?.hide()
        } else if (settings?.startHidden == true) {
            console.log("[Noor-CLI] Hidden & Minimized.")
            win?.hide()
    ...
    if (process.platform != "linux") {
        console.log("[Noor-CLI] Noor For Windows.")
        try {
            if (settings?.autostart == true) {
                if (settings?.startHidden) {
                    app?.setLoginItemSettings({
                        openAtLogin: true,
                        path: path.join(process.resourcesPath, '../Noor.exe'),
                                              args: ['--hidden']
                    });
                } else {
                    app?.setLoginItemSettings({
        ...
        console.log("[Noor-CLI] Noor For Linux.")
